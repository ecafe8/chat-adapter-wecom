...
- Native WeCom streaming via the Chat SDK `Adapter.stream()` hook. Posting an `AsyncIterable<string>` to a thread (e.g. an AI SDK `textStream`) now emits `aibot_respond_msg` stream frames: an initial frame, incremental updates carrying accumulated content, and a final `finish: true` frame.
...
- Configurable update coalescing (`WECOM_STREAM_COALESCE_MS`, default 100 ms) to respect WeCom rate limits.
...
- Added `WECOM_STREAM_COALESCE_MS` (default 100).
